...
function Invoke-ChromeCleanup {
    ...
    $Path1 = "$env:ProgramFiles\Google\Chrome\Application\chrome.exe"
    $Path2 = "${env:ProgramFiles(x86)}\Google\Chrome\Application\chrome.exe"
    $Path3 = "$env:LOCALAPPDATA\Google\Chrome\Application\chrome.exe"

    $ChromeInstalled = (Test-Path $Path1) -or (Test-Path $Path2) -or (Test-Path $Path3)
...
function Invoke-EdgeCleanup {
    ...
    $EdgePath1 = "${env:ProgramFiles(x86)}\Microsoft\Edge\Application\msedge.exe"
    $EdgePath2 = "$env:ProgramFiles\Microsoft\Edge\Application\msedge.exe"

    $EdgeInstalled = (Test-Path $EdgePath1) -or (Test-Path $EdgePath2)
