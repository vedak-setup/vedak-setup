...
function Disable-HubstaffNotifications {
    Write-Host ""
    Write-Host "Disabling Hubstaff screenshot notifications..." -ForegroundColor Yellow

    # Launch Hubstaff hidden briefly so Windows 11 generates its Notification App ID
    ...
    # Turn off Notification Banners in User Registry across all potential Hubstaff AUMIDs
    $aumids = @(
        "Netsoft.Hubstaff",
        "HubstaffClient",
        "Hubstaff",
    ...
    Write-Host "✓ Hubstaff notifications disabled." -ForegroundColor Green
}
...
function Install-Hubstaff {
    ...
    Write-Host "Downloading latest Hubstaff installer (1.9.4+)..." -ForegroundColor Yellow

    $installerPath = "$env:TEMP\HubstaffSetup.exe"
    
    # Download directly using .NET WebClient (prevents progress bar hanging)
